    ...
    //        
            switch(question.name){
                case "happiness":
                    ...
                    color = colorInterpolator((weekly_mean-1)/6).rgb(); // deel door 6 van plaats 4 zodat lampen minder fel staan
                    state.on().rgb(color[0], color[1], color[2]).brightness(100);
                    break;
                case "plezier":
                    state.on().rgb(0, 50, 255).brightness(((weekly_mean-1)/6)*100);
                    break;
                case "resultaat":
                    state.on().rgb(255, 153, 0).brightness(((weekly_mean-1)/6)*100);
                    break;       
                case "uitdaging":
                    state.on().rgb(0, 255, 0).brightness(((weekly_mean-1)/6)*100);
                    break;
                case "werkdruk":
