if (Meteor.isServer) {
    ...
    var hostname = "192.168.0.101",
       ...
       var daily_mean = happiness.daily[0].all.mean;
...
//        });
        ...
        var color = colorInterpolator(daily_mean/5).rgb();
        var color2 = colorInterpolator(1).rgb();
        ...
        api.setLightState(3, state2.on().rgb(color2[0], color2[1], color2[2]))
            .then(displayResult)
            .fail(displayError)
            .done();
